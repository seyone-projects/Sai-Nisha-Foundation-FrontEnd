...
import { Box, Typography, Button, Container, Card, CardContent, useMediaQuery, Divider, CssBaseline, Grid, createTheme, ThemeProvider } from "@mui/material";
...
import ngoImage2 from "../page/image/newborn-baby-crib.jpg"; import ngoImage3 from "../page/image/Baby shower poses.jpg"; import ngoImage6 from "../page/image/home.jpg";
      ...
      <Box sx={{ backgroundColor: DARK_BG, color: WHITE_TEXT, overflowX: "hidden", mt: -4 }}>
        <Box component="section" sx={{ height: "100vh", width: "100%", position: "relative", display: 'flex', alignItems: 'center', backgroundImage: `linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url(${ngoImage6})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
          <Container maxWidth={false} sx={{ textAlign: "center", zIndex: 2 }}><motion.div variants={STAGGER_CONTAINER} initial="initial" animate="animate">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8, duration: 1.5 }}>
              ...
              <Box sx={{ width: '60px', height: '3px', bgcolor: GOLD, margin: '0 auto', mb: isMobile ? 3 : 4 }} /><Typography variant="overline" sx={{ letterSpacing: isMobile ? 4 : 8, display: 'block', mb: 1, color: GOLD, fontSize: isMobile ? '0.6rem' : '0.75rem' }}>Care Moments</Typography>
        ...
        <Box sx={{ py: 0, bgcolor: '#0a0a0a' }}><Container><Typography variant={isMobile ? "h5" : "h4"} textAlign="center" sx={{ color: GOLD, mb: isMobile ? 4 : 6, letterSpacing: isMobile ? 2 : 4, fontWeight: 700, pt: isMobile ? 4 : 6 }}>OUR GOALS</Typography>
          <Grid container spacing={isMobile ? 3 : 6} sx={{ pb: isMobile ? 4 : 6 }}>{goals.map((goal, index) => (
            <Grid item xs={12} md={4} key={index}><motion.div {...FADE_UP} transition={{ delay: index * 0.1 }} whileHover={!isMobile ? { y: -15, boxShadow: `0px 10px 30px ${GOLD}33` } : {}}><Card sx={{ bgcolor: '#1a1a1a', color: '#fff', height: '100%', borderRadius: 0, borderLeft: `2px solid ${GOLD}` }}><CardContent sx={{ p: isMobile ? 3 : 5 }}><Typography variant="body1" sx={{ opacity: 0.7, fontSize: isMobile ? '0.9rem' : '1rem' }}>{goal.text}</Typography></CardContent></Card></motion.div></Grid>
          ))}</Grid></Container></Box>
        <Box sx={{ bgcolor: '#000', py: isMobile ? 4 : 8 }}><Container><Grid container spacing={isMobile ? 4 : 8} alignItems="center"><Grid item xs={12} md={6}><motion.div initial={{ opacity: 0, x: isMobile ? 0 : -100 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}><Typography variant={isMobile ? "h5" : "h4"} sx={{ color: GOLD, mb: 2, fontWeight: 800 }}>When Joy Turns Into Fear</Typography><Divider sx={{ borderColor: GOLD, width: '80px', mb: 3, borderWidth: 2 }} /><Typography variant="body1" sx={{ fontSize: isMobile ? '0.95rem' : '1.1rem', opacity: 0.6 }}>Every day, a mother struggles to reach timely medical care. <br />A newborn waits for life-saving NICU admission. <br />We bridge that gap.</Typography></motion.div></Grid>
        ...
        <Box sx={{ bgcolor: '#0a0a0a', py: isMobile ? 4 : 8 }}><Container><Grid container spacing={isMobile ? 4 : 8} alignItems="center" direction={isMobile ? "column-reverse" : "row"}><Grid item xs={12} md={6} sx={{ textAlign: 'center' }}><motion.div initial="hidden" whileInView="visible" variants={IMAGE_CLIP_REVEAL} viewport={{ once: true }}><Box component="img" src={ngoImage2} sx={{ width: "100%", maxWidth: "500px", borderRadius: '4px', filter: 'grayscale(40%)', height: isMobile ? '300px' : 'auto', objectFit: 'cover' }} /></motion.div></Grid>
          <Grid item xs={12} md={6}><motion.div initial={{ opacity: 0, x: isMobile ? 0 : 100 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}><Typography variant={isMobile ? "h5" : "h4"} sx={{ color: GOLD, mb: 2, fontWeight: 800 }}>Our Core Work</Typography><Divider sx={{ borderColor: GOLD, width: '80px', mb: 3, borderWidth: 2 }} /><Typography variant="body1" sx={{ fontSize: isMobile ? '1rem' : '1.2rem', lineHeight: 2 }}>• Supporting mothers <strong>when it matters most</strong><br />• Helping newborns fight <strong>their first battle</strong><br />• Extending compassion to the unseen</Typography></motion.div></Grid>
        </Grid></Container></Box>
        <Box component={motion.div} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} sx={{ py: isMobile ? 6 : 10, textAlign: "center", px: 2, background: `linear-gradient(rgba(0,0,0,0.85), rgba(0,0,0,0.85)), url(${ngoImage6})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: isMobile ? 'scroll' : 'fixed', borderTop: `1px solid ${GOLD}`, borderBottom: `1px solid ${GOLD}` }}>
          <Container maxWidth="md"><Typography variant={isMobile ? "h6" : "h4"} sx={{ fontStyle: 'italic', color: GOLD, fontWeight: 300 }}>"Sai Nisha Foundation stands beside life, when it needs support the most."</Typography></Container>
        </Box><Footer />
      </Box>
    </ThemeProvider>
  );
