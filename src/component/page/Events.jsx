  ...
  return (
    <>
      ...
      <Box sx={{ position: "fixed", bottom: 20, right: 20, zIndex: 9999 }}>
        <IconButton component="a" href="https://wa.me/919962290875" target="_blank" sx={{ width: 56, height: 56, bgcolor: "#25D366", color: "#fff", "&:hover": { bgcolor: "#1EBE5D", transform: "scale(1.1)" } }}><WhatsAppIcon /></IconButton>
      </Box>

      <Box sx={{ minHeight: "100vh", display: "flex", flexDirection: "column", fontFamily: "Poppins, sans-serif" }}>
        <Box sx={{ minHeight: { xs: "60vh", md: "85vh" }, display: "flex", alignItems: "center", position: "relative", zIndex: 2 }}>
          <CinematicHeroBackground />
          <Container sx={{ position: "relative", zIndex: 10, textAlign: "center" }}>
            <motion.div variants={fadeDown} initial="hidden" animate="visible">
              <Typography variant="h1" sx={{ fontWeight: 900, color: cinematicGold, fontSize: { xs: "2.8rem", md: "6.5rem" }, letterSpacing: { xs: 2, md: 8 }, textTransform: "uppercase", mb: 2 }}>Our EVENTS</Typography>
              <Typography sx={{ fontSize: { xs: 16, md: 22 }, color: "rgba(255,255,255,0.8)", fontStyle: 'italic' }}>"Reliving the moments of joy, talent, and meaningful connections"</Typography>
            </motion.div>
          </Container>
        </Box>
        ...
        <Container maxWidth="md" sx={{ zIndex: 3, mt: -5, mb: 10, textAlign: "center" }}>
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }}>
            <Typography variant="h4" sx={{ color: cinematicGold, mb: 3, fontWeight: 700 }}>The Essence of Our Gathering</Typography>
            <Typography sx={{ color: "rgba(255,255,255,0.7)", lineHeight: 1.8, fontSize: "1.1rem", textAlign: "justify", mx: "auto", px: 2 }}>
             On February 1, 2026, a meaningful "Birthday Tribute & Dream Reveal Ceremony" will be held to honor the legacy and vision of the late Ganesh Shanmugam, Founder of Virtual India ALC Limited. Taking place at the Jayanthi Narayanan Palace in East Tambaram, Chennai, from 4 PM onwards, the event marks a significant step toward fulfilling his lifelong dreams and purpose. The occasion will be graced by several distinguished guests, including spiritual leader Guru Roopa Yogi Sri La Sri Maha Narasimha Athmanatha Swamigal, legendary music director Dhina, film director R. Kannan, and actor Jagan Purushottam. Attendees will also enjoy special cultural performances by groups such as Team Gulab Events and Musica Cura, coming together to celebrate a lasting legacy in an evening of remembrance and inspiration.
            </Typography>
          </motion.div>
        </Container>
        ...
        <Container maxWidth="lg" sx={{ zIndex: 3, pb: 10 }} onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)}>
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 6 }}>
            ...
            <IconButton onClick={handleNext} sx={{ color: '#fff', border: '1px solid rgba(255,255,255,0.2)', display: { xs: 'none', md: 'flex' }, '&:hover': { bgcolor: cinematicGold } }}><ArrowForwardIosIcon /></IconButton>
          </Box>
          ...
          <Box sx={{ display: { xs: 'flex', md: 'none' }, justifyContent: 'center', gap: 4, mt: 6 }}>
             <IconButton onClick={handlePrev} sx={{ bgcolor: 'rgba(255,255,255,0.1)', color: '#fff' }}><ArrowBackIosNewIcon /></IconButton>
             <IconButton onClick={handleNext} sx={{ bgcolor: 'rgba(255,255,255,0.1)', color: '#fff' }}><ArrowForwardIosIcon /></IconButton>
          </Box>
        </Container>
