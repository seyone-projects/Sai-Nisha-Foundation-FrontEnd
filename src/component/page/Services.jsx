...
import { Box, Button, Typography, Grid, Card, CardMedia, CardContent, Container, keyframes, useMediaQuery, CssBaseline, Dialog, DialogTitle, DialogContent, IconButton, createTheme, ThemeProvider } from "@mui/material";
...
const Bubbles = memo(() => (
  <Box sx={{ position: "fixed", inset: 0, zIndex: -1, pointerEvents: "none" }}>
    {[...Array(25)].map((_, i) => <Box key={i} sx={{ position: "absolute", bottom: "-50px", left: `${Math.random() * 100}%`, width: 15, height: 15, borderRadius: "50%", bgcolor: "rgba(214,137,16,0.2)", animation: `float ${12 + Math.random() * 10}s linear infinite`, animationDelay: `${Math.random() * 5}s` }} />)}
...
export default function Services() {
  const isMobile = useMediaQuery("(max-width:600px)"), [sel, setSel] = useState(null), theme = useMemo(() => createTheme({ typography: { fontFamily: "Poppins, sans-serif" } }), []);
  return (
    <ThemeProvider theme={theme}><CssBaseline /><VideoBg /><Bubbles />
      <IconButton href="https://wa.me/919962290875" target="_blank" sx={{ position: "fixed", bottom: 20, right: 20, bgcolor: "#25D366", color: "#fff", zIndex: 1000, p: 1.5, boxShadow: 3, '&:hover': { bgcolor: "#128C7E", transform: 'scale(1.1)' } }}>
      ...
      <Container maxWidth="lg" sx={{ position: "relative", zIndex: 2, overflowX: "hidden" }}>
        <Box sx={{ pt: { xs: 10, md: 15 }, pb: { xs: 6, md: 10 }, textAlign: "center", position: "relative" }}>
          ...
          <Typography variant="h2" sx={{ color: "#fff", fontWeight: 900, textTransform: "uppercase", animation: `${fadeIn} 1s`, fontSize: { xs: '2.2rem', md: '4.5rem' }, textShadow: "0px 4px 15px #000", letterSpacing: { xs: 1, md: 3 } }}>
            Our <span style={{ color: GOLD }}>Services</span>
          </Typography>
          <Typography sx={{ color: MUTED, mt: 2, fontSize: { xs: '0.9rem', md: '1.1rem' }, maxWidth: 600, mx: 'auto', px: 2 }}>Dedicated to providing emergency medical aid, education, and welfare support to those in need.</Typography>
        </Box>
            ...
            <Grid item key={i} xs={12} sm={6} md={4} sx={{ display: "flex", justifyContent: "center", opacity: 0, animation: `${slideUp} .8s ease-out ${i * 0.1}s forwards` }}>
              <Card sx={{ width: "100%", maxWidth: 340, bgcolor: "rgba(255,255,255,0.08)", backdropFilter: "blur(8px)", borderRadius: 6, border: "1px solid rgba(255,255,255,0.1)", color: "#fff", transition: "0.3s", "&:hover": { transform: 'translateY(-10px)', borderColor: GOLD, bgcolor: "rgba(255,255,255,0.15)" } }}>
                ...
                <CardContent sx={{ textAlign: "center", p: 3 }}>
                  ...
                  <Button variant="outlined" onClick={() => setSel(s)} sx={{ color: GOLD, borderColor: GOLD, borderRadius: 10, px: 3, '&:hover': { color: "#fff", borderColor: "#fff", bgcolor: GOLD } }}>Learn More</Button>
                </CardContent>
              </Card>
            </Grid>
          ))}
          ...
          {sel && (
            <>
              <Box sx={{ position: 'relative' }}>
                ...
                <IconButton onClick={() => setSel(null)} sx={{ position: 'absolute', top: 10, right: 10, color: '#fff', bgcolor: 'rgba(0,0,0,0.5)', '&:hover': { bgcolor: GOLD } }}><CloseIcon /></IconButton>
              </Box>
