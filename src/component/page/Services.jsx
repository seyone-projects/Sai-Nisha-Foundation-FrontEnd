...
import {
  Box, Button, Typography, Grid, Card, CardMedia, CardContent, Container, 
  keyframes, useMediaQuery, CssBaseline, Dialog, DialogTitle, DialogContent, 
...
const DATA = [
  ...
  { title: "Mentally Challenged Elders", desc: "Care, dignity, and emotional well-developed.", img: serviceImg3, longDesc: "We provide specialized residential and daycare services for elderly individuals living with dementia and Alzheimer's, offering a safe, loving environment." },
  ...
  { title: "Education", desc: "Empowering children through learning.", img: serviceImg5, longDesc: "Education is the greatest equalizer. We support the schooling of children from low-income backgrounds by providing fees, uniforms, and tools." },
  { title: "Sports", desc: "Encouraging fitness and confidence.", img: serviceImg6, longDesc: "Our sports program aims to identify and nurture athletic talent in underprivileged communities, building discipline and teamwork." },
];

const VideoBg = memo(() => (
  <Box sx={{ position: "fixed", inset: 0, zIndex: -2, bgcolor: "#000", overflow: "hidden" }}>
    <video autoPlay muted loop playsInline src={backgroundVideo} style={{ width: "100%", height: "100%", objectFit: "cover", filter: "brightness(0.4) contrast(1.1)" }} />
  ...
  <Box sx={{ position: "fixed", inset: 0, zIndex: -1, pointerEvents: "none" }}>
    {[...Array(25)].map((_, i) => (
      <Box key={i} sx={{
        ...
        animation: `float ${12 + Math.random() * 10}s linear infinite`, animationDelay: `${Math.random() * 5}s`
      }} />
    ))}
...
const CardItem = ({ s, i, onOpen }) => (
  <Grid item xs={12} sm={6} md={4} sx={{ display: "flex", justifyContent: "center", opacity: 0, animation: `${slideUp} .8s ease-out ${i * 0.1}s forwards` }}>
    <Card sx={{
      width: "100%", maxWidth: 340, bgcolor: "rgba(255,255,255,0.08)", backdropFilter: "blur(8px)", borderRadius: 6, border: "1px solid rgba(255,255,255,0.1)", color: "#fff",
      transition: "0.3s", "&:hover": { transform: 'translateY(-10px)', borderColor: GOLD, bgcolor: "rgba(255,255,255,0.15)" }
    }}>
      ...
      <CardContent sx={{ textAlign: "center", p: 3 }}>
        ...
        <Typography sx={{ color: MUTED, fontSize: '0.85rem', mb: 2, minHeight: '40px' }}>{s.desc}</Typography>
        <Button variant="outlined" onClick={() => onOpen(s)} sx={{ color: GOLD, borderColor: GOLD, borderRadius: 10, px: 3, '&:hover': { color: "#fff", borderColor: "#fff", bgcolor: GOLD } }}>Learn More</Button>
      </CardContent>
    </Card>
  </Grid>
);
...
export default function Services() {
  ...
  const theme = useMemo(() => createTheme({ typography: { fontFamily: "Poppins, sans-serif" } }), []);
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      {/* WhatsApp FAB */}
      <IconButton href="https://wa.me/919962290875" target="_blank" sx={{ position: "fixed", bottom: 20, right: 20, bgcolor: "#25D366", color: "#fff", zIndex: 1000, p: 1.5, boxShadow: '0 4px 12px rgba(0,0,0,0.3)', '&:hover': { bgcolor: "#128C7E", transform: 'scale(1.1)' } }}>
      ...
      <Container maxWidth="lg" sx={{ position: "relative", zIndex: 2 , overflowX: "hidden"}}>
        ...
        <Box sx={{ 
          ...
          <Typography sx={{ color: MUTED, mt: 2, fontSize: { xs: '0.9rem', md: '1.1rem' }, maxWidth: 600, mx: 'auto', px: 2 }}>
        ...
        <Grid container spacing={isMobile ? 3 : 4} justifyContent="center" sx={{ pb: 10 }}>
          {DATA.map((s, i) => <CardItem key={i} s={s} i={i} onOpen={setSel} />)}
        </Grid>

        {/* Details Dialog */}
        <Dialog open={!!sel} onClose={() => setSel(null)} maxWidth="sm" fullWidth PaperProps={{ sx: { bgcolor: "rgba(20,25,35,0.95)", backdropFilter: "blur(20px)", color: "#fff", borderRadius: 5, border: `1px solid ${GOLD}`, m: 2 } }}>
          {sel && (
            <>
              <Box sx={{ position: 'relative' }}>
                <CardMedia component="img" height={isMobile ? "200" : "280"} image={sel.img} sx={{ objectFit: 'cover' }} />
                <IconButton onClick={() => setSel(null)} sx={{ position: 'absolute', top: 10, right: 10, color: '#fff', bgcolor: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', '&:hover': { bgcolor: GOLD } }}><CloseIcon /></IconButton>
              </Box>
              <DialogTitle sx={{ color: GOLD, fontWeight: 800, fontSize: { xs: '1.3rem', md: '1.6rem' }, pt: 3 }}>{sel.title}</DialogTitle>
              <DialogContent sx={{ pb: 4 }}><Typography sx={{ lineHeight: 1.8, color: MUTED, fontSize: '0.95rem' }}>{sel.longDesc}</Typography></DialogContent>
            </>
          )}
        </Dialog>

        {/* Footer Container */}
        <Box sx={{ width: "100%", mt: 5, borderTop: "1px solid rgba(255,255,255,0.1)", pt: 5, "& *": { color: "#fff !important" } }}>
          <Footer />
        </Box>
      </Container>
    </ThemeProvider>
